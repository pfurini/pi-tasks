...
### Added
- **Custom widget sort orders.** `sortOrder` now also accepts a *sort spec* — an ordered list of comparison keys (`id` / `status` / `updatedAt`), each with an optional `direction` and, on `status`, a custom `rank` — written straight into `tasks-config.json` at either scope. A new `active` preset (in-progress → pending → completed, then by id) covers the common case from `/tasks` → Settings without editing a file; it is the grouping the existing `status` preset reverses, and the one the `TaskList` tool already uses. Presets are themselves specs, so every built-in order is a starting point to copy and adjust. Specs are pure data and validated when read: an unrecognised field, direction or status falls back to `id` order rather than breaking the widget, and a spec appears in the settings menu as a read-only `custom` value that picking a preset replaces. That validation also tightens an unrecognised `sortOrder` *name* — a typo previously selected no comparator at all and left tasks in whatever order the store happened to hold them, which was creation order in practice but not guaranteed; it now sorts by `id` explicitly. Deliberately no executable config file — `.pi/` lives inside cloned repositories, so nothing there can run on your machine. (Converges [#35](https://github.com/tintinweb/pi-tasks/pull/35) — thanks [@Minh-Ng](https://github.com/Minh-Ng) — and [#54](https://github.com/tintinweb/pi-tasks/pull/54) — thanks [@Rizhiy](https://github.com/Rizhiy))
- **Completed tasks can be collapsed to a single line.** `collapseCompleted` (default `false`, toggleable from `/tasks` → Settings) replaces finished tasks with one `✔ N completed` line at the bottom of the widget. It composes with the visible limit rather than overriding it: collapsing decides what goes in the list, then `maxVisible` / `showAll` / `hiddenAt` apply to whatever is left, so a long backlog stays bounded. The header line still counts every task. ([#54](https://github.com/tintinweb/pi-tasks/pull/54) — thanks [@Rizhiy](https://github.com/Rizhiy))
- **A customization guide** — [CUSTOMIZING.md](CUSTOMIZING.md), linked from the README: how global and project config merge, what each display setting does and in what order they apply, the sort presets, how to write a spec, worked recipes, and the failure modes worth knowing about.

### Fixed
- **An unrecognised `hiddenAt` value no longer blanks the task widget.** The setting indexes a lookup of two truncation functions, so a hand-edited `tasks-config.json` with anything other than `top` or `bottom` produced `undefined` and threw on every render. The widget's render guard caught it, which meant the widget silently showed nothing at all — with tasks in the list and no error anywhere — until the file was corrected. The value is now narrowed to `bottom` unless it is exactly `top`, matching the documented default.
